...
<div class="wrapper">
  ...
  <div class="card-body">
    ...
    <h6 class="card-subtitle mb-2 text-muted">Nama Perusahaan : {{ $student->nama_perusahaan}}</h6>
    <p class="card-text">Lokasi : {{ $student->lokasi_magang}}</p>
	@if($student->status == 1)
		<p class="card-text">Status : <label class="badge bg-warning">{{ $student->statuss->nama}}</label></p>
    @else
		<p class="card-text">Status : <label class="badge bg-success">{{ $student->statuss->nama}}</label></p>
	@endif
    <a href="{{$student->id}}/editdata" class="btn btn-primary">Edit</a>
